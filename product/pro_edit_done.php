...
// ３．SQL文を用意(データ更新：UPDATE)
    $stmt = $dbh->prepare(
    "UPDATE product SET productname = :productname, price = :price, picture = :picturenewname WHERE productcode = :productcode"
    );
...
// 4. バインド変数を用意
    ...
    $stmt->bindValue(':picturenewname', $picturenewname, PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)
